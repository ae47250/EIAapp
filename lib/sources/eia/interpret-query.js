...
export async function interpretQuery(query, countries = [], options = {}) {
  const forms = buildQueryForms(query);
  const aiResult = await interpretQueryWithOpenAI(forms, countries, options);
...
async function interpretQueryWithOpenAI(forms, countries, options = {}) {
  ...
  const includeCleanedQuery = options.includeCleanedQueryInPrompt !== false;
  const prompt = [
    ...
    includeCleanedQuery ? `Lightly cleaned query: ${JSON.stringify(forms.cleanedQuery)}` : null
  ].filter(Boolean).join("\n");
...
export function validateAiInterpretation(parsed, query, countries = [], fallbackIntent = null) {
  ...
  const ruleExclusions = mergeExclusions(ruleIntent.exclusions, findRuleExclusions(normalizeText(validationQuery)));
  ...
  const countryField = validateAiField("country", readAiField(parsed, "country", ["countryCode", "countryName"]), confidence, value => {
    ...
    return isGeographySupportedByQuery(code, validationQuery, ruleIntent, countryList) ? code : null;
  }, ruleIntent.fields.country);
  const productField = describeProductBreadth(validateAiField(
    "product",
    readAiField(parsed, "product", ["product"]),
    confidence,
    value => validatePositiveField("product", value, candidate => validateExplicitCategory(candidate, PRODUCT_RULES, PRODUCT_ALIASES, validationQuery)),
    ruleIntent.fields.product,
    {
      ...
      fallbackBeforeAmbiguity: !BROAD_PRODUCT_OPTIONS.has(ruleIntent.fields.product?.normalizedValue)
        || hasPhrase(validationQuery, ruleIntent.fields.product?.normalizedValue),
  ...
  const activityField = validateAiField(
    ...
    confidence,
    value => validatePositiveField("activity", value, candidate => validateExplicitCategory(candidate, ACTIVITY_RULES, ACTIVITY_ALIASES, validationQuery)),
  ...
  const sectorField = validateAiField("sector", {
    ...
    const explicitSector = sector && isCategoryExplicit(sector, SECTOR_RULES, validationQuery) ? sector : null;
  ...
  const validatedGeographies = validateAiGeographies(parsed.geographies, confidence, parsed, countryList, validationQuery, ruleIntent);
  ...
  const conceptPairs = preserveAmbiguousPairProducts(
    validateAiConceptPairs(parsed.conceptPairs, confidence, validationQuery, ruleIntent),
    productField
  );
  ...
  const unknownQualifiers = validateUnknownQualifiers(parsed.unknownQualifiers, validationQuery, ruleIntent);
...
function interpretProductWithRules(normalizedQuery, exclusions = []) {
  const direct = firstNonExcludedRuleMatch(normalizedQuery, PRODUCT_RULES, exclusions, "product");
  if (direct) return {
    value: direct,
    reason: null,
