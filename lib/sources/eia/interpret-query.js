...
export function normalizeSubmittedIntent(rawIntent, query, countries = []) {
  ...
  const country = findCountryByCode(countries, rawIntent.countryCode) || fallbackIntent.country || null;
  const product = normalizeAllowed(rawIntent.product, PRODUCT_RULES.map(rule => rule.value)) || fallbackIntent.product;
  const activity = normalizeAllowed(rawIntent.activity, ACTIVITY_RULES.map(rule => rule.value)) || fallbackIntent.activity;
  const frequency = normalizeAllowed(rawIntent.frequency, FREQUENCY_RULES.map(rule => rule.value)) || fallbackIntent.frequency;
...
function resolveAiCountry(parsed, countries) {
  ...
  const matched = countries.find(country => normalizeText(country.name) === normalizedName) ||
    countries.find(country => hasPhrase(normalizeText(country.name), normalizedName));
  if (matched) return matched;
  const countryCode = String(parsed.countryCode || "").trim().toUpperCase();
  return /^[A-Z0-9]{3}$/.test(countryCode)
    ? { code: countryCode, name: String(parsed.countryName || countryCode).trim() }
    : null;
}
