...
export function rankRetrieval(intent, retrieval, routeFamilyOverride = null) {
  ...
  annotateEquivalentChoices(accepted);
...
function scoreCandidate(intent, retrieval, prepared, context) {
  ...
  const aggregation = aggregationCompatibility(intent, retrieval, candidate, productMatch, activityMatch);
...
function aggregationCompatibility(intent, retrieval, candidate, productMatch, activityMatch) {
  ...
  const detectedSectors = detectVocabulary(areas, SECTOR_VOCABULARY, ["title", "sector", "facets"]);
  const requestedSector = retrieval?.concept?.sector || intent?.sector;
  const requestedActivity = retrieval?.concept?.activity || intent?.activity;
  const sectorSpecific = !requestedSector && (
    detectedSectors.size > 0 ||
    (KEYWORDS.sectorSpecific || []).some(keyword => phraseIncludes(text, keyword))
  );
  const officialTotalRole = officialTotalRoleForCandidate(candidate);
  const exactConcepts = productMatch.compatibility >= 0.99 && (!requestedActivity || activityMatch.compatibility >= 0.99);
  const missingActivityWarning = requestedActivity ? [] : ["activity_missing_ranked_aggregate_first"];
  ...
  if (!requestedSector && officialTotalRole === "exact_total" && allSectors) {
    return { compatibility: 1, reasonCodes: ["official_all_sectors_total_priority"], warnings: missingActivityWarning };
  }
  if (officialTotalRole === "exact_total" && exactConcepts) {
    return { compatibility: RANKING_CONFIG.aggregation.officialTotalLabel, reasonCodes: ["official_total_label"], warnings: missingActivityWarning };
  }
  if (officialTotalRole) {
    return { compatibility: RANKING_CONFIG.aggregation.officialAggregateLabel, reasonCodes: [`official_aggregate_label_${officialTotalRole}`], warnings: missingActivityWarning };
  }
  if (aggregate) {
...
function officialTotalRoleForCandidate(candidate) {
  const selector = candidate?.selector || {};
  for (const rule of ROUTING_CONFIG.officialTotalSelectors || []) {
    if (normalizeText(rule.route) !== normalizeText(selector.route)) continue;
    if (rule.facets && !Object.entries(rule.facets).every(([name, value]) => String(selector.facets?.[name] || "") === String(value))) continue;
    if (rule.facet) {
      const value = String(selector.facets?.[rule.facet] || "");
      if (rule.values && !rule.values.includes(value)) continue;
      if (rule.pattern && !new RegExp(rule.pattern).test(value)) continue;
    }
    return rule.role || "aggregate";
  }
  return null;
}
...
function specificitySignals(intent, retrieval, candidate, coverage) {
  ...
  const officialTotalLabel = officialTotalRoleForCandidate(candidate) ? 1 : 0;
  ...
  if (noSubtypeRequested && broadScope) reasonCodes.push("broad_scope_preferred_no_subtype_requested");
...
function compareSemanticTieSignals(left, right) {
  ...
  const comparisons = [
    ...
    [rightSignals.noSubtypeRequested && rightSignals.broadScope ? 1 : 0, leftSignals.noSubtypeRequested && leftSignals.broadScope ? 1 : 0],
    [rightSignals.officialTotalLabel, leftSignals.officialTotalLabel],
...
function annotateEquivalentChoices(candidates) {
  let groupNumber = 0;
  for (let start = 0; start < candidates.length;) {
    let end = start + 1;
    while (
      end < candidates.length &&
      candidates[end].ranking.tier === candidates[start].ranking.tier &&
      candidates[end].ranking.score === candidates[start].ranking.score &&
      compareSemanticTieSignals(candidates[start], candidates[end]) === 0
    ) end += 1;
    if (end - start > 1) {
      const groupId = `equivalent-choice-${++groupNumber}`;
      for (const candidate of candidates.slice(start, end)) {
        candidate.ranking.signals.equivalentChoiceGroup = groupId;
        candidate.ranking.reasonCodes = dedupePreserveOrder([...candidate.ranking.reasonCodes, "equivalent_semantic_choice"]);
      }
    }
    start = end;
  }
