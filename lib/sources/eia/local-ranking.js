...
const PRODUCT_ALIASES = buildAliasMap(ROUTING_CONFIG.vocabulary?.products);
const ACTIVITY_ALIASES = buildAliasMap(ROUTING_CONFIG.vocabulary?.activities);
const SECTOR_ALIASES = buildAliasMap(ROUTING_CONFIG.vocabulary?.sectors);
const UNIT_ALIASES = buildAliasMap(ROUTING_CONFIG.vocabulary?.units);
const DESCRIPTION_ONLY_ACTIVITIES = new Set(
  Object.entries(ROUTING_CONFIG.vocabulary?.activities || {})
    .filter(([, config]) => config.titleOrMeasureRequired)
...
function assessEligibility(retrieval, candidate) {
  ...
  if (selectorGeography && selectorGeography !== targetGeography) {
    warnings.push("selector_geography_mismatch");
    return { eligible: false, effectivePool: null, warnings };
  }
...
function scoreCandidate(intent, retrieval, candidate, pool, prepared) {
  ...
  const activitySignal = scoreActivity(intent, retrieval, candidate);
  ...
  const specificitySignal = scoreUnrequestedSpecificity(intent, retrieval, candidate);
  score += specificitySignal.score;
  reasons.push(...specificitySignal.reasons);
  warnings.push(...specificitySignal.warnings);

  const activityInferenceSignal = scoreActivityInference(intent, retrieval);
  score += activityInferenceSignal.score;
  reasons.push(...activityInferenceSignal.reasons);
  warnings.push(...activityInferenceSignal.warnings);
  ...
  const normalizedActivity = normalizeText(effectiveActivity(intent, retrieval) || "");
  if (DESCRIPTION_ONLY_ACTIVITIES.has(normalizedActivity) && candidateText(candidate, ["title", "measure"]).includes(normalizedActivity) === false && candidateText(candidate, ["description"]).includes(normalizedActivity)) {
    score += WEIGHTS.misleadingDescriptionPenalty || 0;
    reasons.push("activity_only_in_description_penalty");
  }

  const derivedMetricSignal = scoreDerivedMetric(intent, candidate);
  score += derivedMetricSignal.score;
  reasons.push(...derivedMetricSignal.reasons);
  warnings.push(...derivedMetricSignal.warnings);
...
function scoreDerivedMetric(intent, candidate) {
  const combinedText = candidateText(candidate, ["title", "measure", "description", "conceptType"]);
  const queryText = normalizeText(intent?.correctedQuery || intent?.normalizedQuery || intent?.originalQuery || "");
  const derivedMatch = (KEYWORDS.derivedMetric || []).find(keyword => phraseIncludes(combinedText, keyword));
  if (!derivedMatch || phraseIncludes(queryText, derivedMatch)) return { score: 0, reasons: [], warnings: [] };
  return {
    score: WEIGHTS.derivedMetricPenalty || 0,
    reasons: ["derived_metric_penalty"],
    warnings: ["derived_metric_not_requested"]
  };
}

function scoreActivityInference(intent, retrieval) {
  if (intent?.activity) return { score: 0, reasons: [], warnings: [] };
  if (retrieval?.concept?.activitySource === "weak_inference") {
    const inference = retrieval.concept.activityInference || intent?.activityInference || {};
    return {
      score: WEIGHTS.missingActivityFallbackPenalty || 0,
      reasons: [inference.reasonCode || "activity_inferred_from_weak_hint", "missing_activity_fallback_penalty"],
      warnings: [inference.warning || "No explicit activity was found; a weak activity hint was used for ranking."]
    };
  }
  if (retrieval?.concept?.activitySource === "missing") {
    return {
      score: WEIGHTS.missingActivityFallbackPenalty || 0,
      reasons: ["activity_missing_fallback"],
      warnings: ["No explicit activity was found; results are ranked as fallback suggestions."]
    };
  }
  return { score: 0, reasons: [], warnings: [] };
}

function scoreUnrequestedSpecificity(intent, retrieval, candidate) {
  const reasons = [];
  const warnings = [];
  let score = 0;
  const title = candidateText(candidate, ["title"]);
  const product = normalizeText(intent?.product || "");
  const activity = normalizeText(intent?.activity || "");
  const geographyName = normalizeText(retrieval?.geography?.name || candidate?.geography?.name || "");
  const geographyCode = normalizeText(retrieval?.geography?.code || candidate?.geography?.code || "");

  if (!intent?.sector && (KEYWORDS.unrequestedSector || KEYWORDS.sector || []).some(keyword => phraseIncludes(title, keyword))) {
    score += WEIGHTS.unrequestedSectorPenalty || 0;
    warnings.push("sector_specific_not_requested");
    reasons.push("unrequested_sector_penalty");
  }
  if (!intent?.sector && (KEYWORDS.allSectorsAggregate || []).some(keyword => phraseIncludes(title, keyword))) {
    score += WEIGHTS.allSectorsAggregateBoost || 0;
    reasons.push("all_sectors_aggregate_boost");
  }

  const compactTitle = normalizeText(title);
  const exactNameTitle = normalizeText(`${product} ${activity} ${geographyName}`);
  const exactCodeTitle = normalizeText(`${product} ${activity} ${geographyCode}`);
  if (product && activity && (compactTitle === exactNameTitle || compactTitle === exactCodeTitle)) {
    score += WEIGHTS.unqualifiedAggregateBoost || 0;
    reasons.push("unqualified_aggregate_boost");
  }
...
function scoreActivity(intent, retrieval, candidate) {
  const areas = candidateAreas(candidate);
  const terms = buildTermEntries([effectiveActivity(intent, retrieval)], ACTIVITY_ALIASES);
...
function buildAliasMap(items = {}) {
  return new Map(Object.entries(items).map(([value, config]) => [normalizeText(value), [value, ...(config.terms || [])]]));
}
