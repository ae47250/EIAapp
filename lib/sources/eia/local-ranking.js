...
function scoreCandidate(intent, retrieval, prepared, context) {
  ...
  const lexicalQualifiers = lexicalQualifierSignals(intent, retrieval, candidate, coverage);
...
function compareRankedCandidates(left, right) {
  ...
  const semanticDifference = compareHierarchyNeutralTieSignals(left, right);
...
function lexicalQualifierSignals(intent, retrieval, candidate, coverage) {
  const requestedProduct = normalizeText(retrieval?.concept?.product || intent?.product || intent?.scope);
  ...
  const requestedQualifiers = matchingLexicalQualifiers(definition, query);
  const candidateQualifiers = matchingLexicalQualifiers(definition, text);
  ...
  if (exactRequestedQualifierMatches > 0) reasonCodes.push("requested_lexical_qualifier_match");
...
function matchingLexicalQualifiers(definition, text) {
  return Object.entries(definition.lexicalQualifiers || {})
    .filter(([, terms]) => terms.some(term => phraseIncludes(text, term)))
    .map(([qualifier]) => qualifier);
}

function compareHierarchyNeutralTieSignals(left, right) {
  const leftSignals = left?.ranking?.signals?.lexicalQualifiers || {};
  const rightSignals = right?.ranking?.signals?.lexicalQualifiers || {};
  const comparisons = [
