...
export function buildStructuredIntent(baseIntent, query) {
  ...
  const interpretedActivity = firstMentionValue(conceptMentions, "activity") || approvedFieldValue(baseIntent, "activity") || null;
  const activity = canonicalizeActivity(product, interpretedActivity);
  const sector = approvedFieldValue(baseIntent, "sector") || firstRuleValue(normalizedQuery, SECTOR_RULES) || null;
  const conceptPairResolution = buildOrderedConceptPairs(baseIntent, conceptMentions, product, interpretedActivity, normalizedQuery);
  const inheritedPairs = Array.isArray(baseIntent?.validatedConceptPairs) ? baseIntent.validatedConceptPairs : [];
  const conceptPairs = conceptPairResolution.pairs.map((pair, index) => canonicalizeConceptPair(pair, inheritedPairs[index]));
  ...
  const multiCountryComparison = buildMultiCountryComparisonStatus(geographyMentions, route);
  const frequency = validateFrequency(effectiveFrequency, route.family, explicitFrequency || Boolean(baseIntent?.frequencyExplicit));
  const ambiguityReasons = buildAmbiguityReasons(geographyMentions, conceptMentions, conceptPairs, conceptPairResolution, route, baseIntent?.fields, unknownQualifiers);
  ...
  return {
    ...
    sector,
    conceptPairs,
    conceptPairStatus: conceptPairResolution.status,
    conceptPairRule: conceptPairResolution.rule,
    ...
    validation: {
      ...
      geographyConflicts: Array.isArray(baseIntent?.geographyConflicts) ? baseIntent.geographyConflicts : [],
    ...
    provenance: {
      ...
      geographyEvidence: geographyMentions.map(mention => mention.evidence || {
        code: mention.geography.code,
        rawText: mention.text,
        source: mention.source || "query",
        conflictStatus: "none"
      }),
      concepts: conceptPairs.map(pair => pair.source),
      canonicalizations: conceptPairs
        .filter(pair => pair.activityNormalization)
        .map(pair => ({
          field: "activity",
          from: pair.originalActivity,
          to: pair.activity,
          rule: pair.activityNormalization
        })),
...
function canonicalizeActivity(product, activity) {
  return product === "electricity" && activity === "production" ? "generation" : activity;
}

function canonicalizeConceptPair(pair, inheritedPair = null) {
  if (inheritedPair?.activityNormalization && inheritedPair.activity === pair?.activity) {
    return {
      ...pair,
      originalActivity: inheritedPair.originalActivity,
      activityNormalization: inheritedPair.activityNormalization
    };
  }
  const activity = canonicalizeActivity(pair?.product, pair?.activity);
  if (activity === pair?.activity) return pair;
  return {
    ...pair,
    activity,
    originalActivity: pair.activity,
    activityNormalization: "electricity_production_to_generation"
  };
}

function buildMultiCountryComparisonStatus(geographyMentions, route) {
  const geographies = geographyMentions.map(mention => mention.geography);
  const countryGeographies = geographies.filter(geography => ["country", "national"].includes(geography?.type));
  const active = countryGeographies.length >= 2 && countryGeographies.length === geographies.length;
  return {
    active,
    status: active ? "active" : geographies.length >= 2 ? "not_eligible" : "not_applicable",
    geographyCodes: countryGeographies.map(geography => geography.code),
    routeFamily: route.family,
    reason: active
      ? "two_or_more_validated_countries"
      : geographies.length >= 2
        ? "request_contains_non_country_geography"
        : "fewer_than_two_validated_countries"
  };
}
...
function selectGeographyMentions(baseIntent, scanned, authoritativeAi) {
  ...
  const evidenceByCode = new Map((baseIntent?.geographyEvidence || []).map(item => [item.code, item]));
  const mentions = values.map((value, order) => {
    const geography = resolveApprovedGeography(value?.code || value?.name || value);
    const evidence = evidenceByCode.get(geography?.code);
    return geography ? {
      index: Number.isFinite(Number(evidence?.index)) ? Number(evidence.index) : order,
      text: evidence?.rawText || null,
      source: evidence?.source || "validated_interpreter",
...
function selectConceptMentions(baseIntent, scanned, exclusions, authoritativeAi) {
  ...
  if (baseIntent?.conceptPairStatus === "unresolved") return scanned.filter(mention => !isExcluded(mention));
...
function normalizeExclusions(values) {
  ...
  return values.map((item, order) => {
    ...
    seen.add(`${type}:${value}`);
    const corrections = (Array.isArray(item?.corrections) ? item.corrections : []).map(correction => ({
      type: normalizeText(correction?.type),
      value: normalizeText(correction?.value),
      source: correction?.source || null,
      confidence: correction?.confidence ?? null,
      from: normalizeText(correction?.from),
      to: normalizeText(correction?.to),
      editDistance: Number.isFinite(Number(correction?.editDistance)) ? Number(correction.editDistance) : null
    })).filter(correction => correction.type && correction.value && correction.from && correction.to && correction.editDistance !== null);
    return {
      type,
      value,
      order: Number.isFinite(Number(item?.order)) ? Number(item.order) : order,
      confidence: item?.confidence ?? null,
      source: item?.source || "validated_interpreter",
...
function buildOrderedConceptPairs(baseIntent, mentions, fallbackProduct, fallbackActivity, normalizedQuery) {
  if (baseIntent?.conceptPairStatus === "unresolved") {
    return unresolvedConceptPairs();
  }
  const validated = Array.isArray(baseIntent?.validatedConceptPairs) ? baseIntent.validatedConceptPairs : [];
  if (validated.length > 0) {
    return resolvedConceptPairs(validated.map((pair, order) => ({
      order,
  ...
  products = collapseCompoundCarrierProducts(products, activities, normalizedQuery);
  ...
  if (products.length === 1 && effectiveActivities.length > 1) {
    ...
    rule = "shared_product_multiple_activities";
  } else if (products.length > 1 && effectiveActivities.length === 1) {
    if (!isSharedTrailingActivity(products, effectiveActivities[0])) return unresolvedConceptPairs();
    pairs = products.map(product => ({ product, activity: effectiveActivities[0] }));
    rule = "shared_trailing_activity";
  } else if (products.length > 1 && effectiveActivities.length > 1) {
    if (!hasOrderedOneToOneScope(products, effectiveActivities)) return unresolvedConceptPairs();
    pairs = products.map((product, index) => ({ product, activity: effectiveActivities[index] }));
    rule = "ordered_one_to_one_pairs";
  } else {
    pairs = products.map(product => ({ product, activity: nearestMention(product, effectiveActivities) }));
    rule = pairs.length > 0 ? "single_pair" : null;
  }
  if (pairs.length === 0 && (baseIntent?.productAlternatives?.length || baseIntent?.fields?.product?.alternatives?.length)) {
    ...
    rule = "approved_product_alternatives";
  }

  const seen = new Set();
  const resolved = pairs.map((pair, order) => {
  ...
  return resolvedConceptPairs(resolved, rule);
}

function resolvedConceptPairs(pairs, rule) {
  const complete = pairs.length > 0 && pairs.every(pair => pair.product && pair.activity);
  return {
    pairs,
    status: complete
      ? "validated"
      : rule === "approved_product_alternatives" && pairs.some(pair => pair.activity)
        ? "approved_alternatives"
        : pairs.length > 0 ? "partial" : "missing",
    rule: rule || null
  };
}

function unresolvedConceptPairs() {
  return {
    pairs: [],
    status: "unresolved",
    rule: "clarification_required_multiple_products_and_activities"
  };
}

function isSharedTrailingActivity(products, activity) {
  return Number.isFinite(activity?.index) && products.every(product => Number(product.index) < Number(activity.index));
}

function hasOrderedOneToOneScope(products, activities) {
  if (products.length !== activities.length) return false;
  return products.every((product, index) => {
    const activity = activities[index];
    const nextProduct = products[index + 1];
    return Number(product.index) < Number(activity.index)
      && (!nextProduct || Number(activity.index) < Number(nextProduct.index));
  });
}

function collapseCompoundCarrierProducts(products, activities, normalizedQuery) {
  if (products.length < 2) return products;
  const rules = ROUTING_CONFIG.compoundProductRules || [];
  return products.filter(product => !rules.some(rule => shouldCollapseCarrierProduct(
    product,
    products,
    activities,
    normalizedQuery,
...
function shouldCollapseCarrierProduct(product, products, activities, normalizedQuery, rule) {
  if (product.value !== rule.carrier || !rule.preferSpecificProduct) return false;
  if (rule.activity && !activities.some(activity => activity.value === rule.activity)) return false;
  if (rule.genericTerms?.length && !rule.genericTerms.map(normalizeText).includes(normalizeText(product.text))) return false;
  const specificProducts = products.filter(other => other !== product && other.value !== rule.carrier);
  if (specificProducts.length === 0) return false;
  if (!rule.requireAdjacentSpecificProduct) return true;
  return specificProducts.some(other => mentionsAreAdjacent(other, product, normalizedQuery));
}

function mentionsAreAdjacent(left, right, normalizedQuery) {
  if (!left?.text || !right?.text || !Number.isFinite(left.index) || !Number.isFinite(right.index)) return false;
  if (left.index >= right.index) return false;
  const leftEnd = left.index + left.text.length;
  return normalizeText(String(normalizedQuery || "").slice(leftEnd, right.index)) === "";
}
...
export function findGeographyMentions(normalizedQuery, originalQuery = "") {
  ...
  return selectOrderedMentions(disambiguateStateCountryMatches(matches, normalizedQuery), "geography");
}

function disambiguateStateCountryMatches(matches, normalizedQuery) {
  const groups = new Map();
  for (const match of matches) {
    const key = `${match.index}:${match.length}:${match.text}`;
    const group = groups.get(key) || [];
    group.push(match);
    groups.set(key, group);
  }

  const internationalHints = ROUTING_CONFIG.geographyDisambiguation?.internationalHints || [];
  const hasInternationalHint = internationalHints.some(hint => hasPhrase(normalizedQuery, hint));
  const selectedCodes = new Map();
  for (const [key, group] of groups) {
    const state = group.find(match => match.type === "state");
    const country = group.find(match => match.type === "country");
    if (!state || !country) continue;
    const hasOtherForeignGeography = matches.some(match =>
      match.type !== "state" && !isUnitedStatesMatch(match) && `${match.index}:${match.length}:${match.text}` !== key
    );
    const hasUnitedStates = matches.some(match =>
      isUnitedStatesMatch(match) && `${match.index}:${match.length}:${match.text}` !== key
    );
    const preferCountry = hasOtherForeignGeography || (hasInternationalHint && !hasUnitedStates);
    selectedCodes.set(key, preferCountry ? country.code : state.code);
  }

  return matches.filter(match => {
    const key = `${match.index}:${match.length}:${match.text}`;
    return !selectedCodes.has(key) || selectedCodes.get(key) === match.code;
  });
}

function isUnitedStatesMatch(match) {
  return ["US", "USA"].includes(String(match?.code || "").toUpperCase()) || normalizeText(match?.name) === "united states";
}
...
function buildAmbiguityReasons(geographies, concepts, conceptPairs, conceptPairResolution, route, fields = {}, unknownQualifiers = []) {
  const reasons = [];
  ...
  if (!conceptPairs.some(pair => pair.activity) && !concepts.some(mention => mention.type === "activity" || mention.type === "scope")) reasons.push("activity_or_scope_missing");
  if (conceptPairResolution.status === "unresolved") reasons.push("concept_pair_scope_unresolved");
