...
const CONCEPT_RULES = [
  { type: "product", value: "natural gas", terms: ["natural gas plant liquids", "natural gas liquids", "dry natural gas", "natural gas", "nat gas"] },
...
function chooseRoute({ geographyMentions, primaryGeography, conceptMentions, product, activity, requestedFrequency }) {
  ...
  if (hasState && product === "total energy") {
    return decision("seds", "A state-level total-energy request uses SEDS.");
  }
  if (hasState && electricitySpecific) {
    return decision("domestic", "A state-level electricity request uses Domestic EIA.");
  }
  if (hasState && requestedFrequency !== "annual") {
    return decision("domestic", "A nonannual state request uses Domestic EIA.");
  }
