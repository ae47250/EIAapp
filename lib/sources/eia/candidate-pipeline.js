...
export const CANDIDATE_PIPELINE_VERSION = "phase4a-v3";
...
export async function buildLocalCandidatePipeline(input, options = {}) {
  ...
  const retrievals = finalRanked.retrievals.map(retrieval => addPresentationStatus(retrieval, intent));
...
function addPresentationStatus(retrieval, intent) {
  ...
  if (!retrieval?.concept?.activity) {
    warnings.push({
      code: "activity_missing_aggregate_priority",
      message: "No activity was requested. Results are grouped by activity and measure type; within each group, official total-labeled series rank before sector-specific or component series."
    });
  }
