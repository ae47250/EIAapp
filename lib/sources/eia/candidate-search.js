...
export default async function candidateSearchHandler(req, res) {
  ...
  try {
    ...
    const candidateGroups = buildPublicGroups(pipeline.retrievals || []);
...
function buildPublicGroups(retrievals) {
  const groups = [];
  ...
  for (const retrieval of retrievals) {
    for (const group of retrieval.interpretationGroups || []) {
      ...
      const candidates = group.candidates.slice(0, remaining).map(candidate => serializeCandidate(candidate, retrieval, group));
...
function serializeCandidate(candidate, retrieval, group) {
  return {
    ...
    equivalentChoiceGroup: candidate.ranking?.signals?.equivalentChoiceGroup || null,
