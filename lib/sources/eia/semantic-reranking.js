...
const EXACT_AGGREGATE_REASON = "exact_verified_aggregate";
const TIER_A_AGGREGATE_REASONS = new Set(["all_sectors_aggregate_priority"]);
...
export function evaluateSemanticRerankingTrigger(input, retrieval, options = {}) {
  ...
  if (options.triggerPolicy !== "always" && CONFIG.trigger.protectExactVerifiedAggregate && isProtectedAggregate(families[0])) {
    return triggerResult(false, "decisive_exact_aggregate", familyIds);
  }
...
export function buildSemanticRerankingRequest(input, retrieval, allowedFamilyIds = null) {
  ...
  return {
    ...
    lightlyCleanedQuery: intent.cleanedQuery || intent.normalizedQuery || "",
    structuredIntent: {
...
function isProtectedAggregate(candidate) {
  const reasons = candidate?.ranking?.reasonCodes || [];
  return reasons.includes(EXACT_AGGREGATE_REASON) || (
    candidate?.ranking?.tier === "A" && reasons.some(reason => TIER_A_AGGREGATE_REASONS.has(reason))
  );
}
