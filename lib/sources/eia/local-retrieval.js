...
function classifyCandidates(records, family, concept, frequency) {
  ...
  for (const record of records) {
    ...
    const pool = frequency.mode === "fallback" || alternativeProductMatch || tier === "partial_tokens" || tier === "trigram"
      ? "fallback"
      : "primary";
    const reasonCodes = [
...
async function loadAndValidateFamily(family) {
  const [compressed, manifestText, validationText] = await Promise.all([
    readFileAsync(new URL(`${family}.jsonl.gz`, INDEX_ROOT)),
...
function matchesActivity(record, document, terms) {
  ...
  if (constrainedActivity) {
    return titleOrMeasureIncludes(title, measure, constrainedActivity[0]);
  }
...
function titleOrMeasureIncludes(title, measure, term) {
  const normalized = normalizeText(term);
  return measure.includes(normalized) || title.startsWith(`${normalized} `) || title.startsWith(`net ${normalized} `);
}
