...
export const SESSION_DURATION_SECONDS = 20 * 60;
...
export function isLoginRequired(env = process.env) {
  return String(env.LOGIN_REQUIRED || "").trim().toLowerCase() !== "off";
}
...
export function requireAuthentication(req, res, env = process.env) {
  if (!isLoginRequired(env)) return true;
