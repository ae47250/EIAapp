...
export default async function handler(req, res) {
  ...
  try {
    ...
    const intent = await interpretQuery(query, countries);
...
async function buildSingleCountrySearch({ apiKey, country, query, intent, frequency }) {
  ...
  if (rawCandidates.length === 0) {
    return { query, country, intent, selectedSeries: null, variables: [], userMessage: "EIA returned rows, but no usable numeric series were found. Try a broader search such as Brazil energy consumption." };
  }
  ...
  return {
    ...
    note: "Variables are grouped by product and activity. One preferred unit is shown for each variable, and coverage is computed from actual observations."
  };
}
...
async function enrichCandidates({ apiKey, country, candidates, frequency }) {
  const settled = await Promise.allSettled(candidates.map(candidate => fetchExactSeries({
    apiKey,
    country,
  ...
  return settled.map((result, index) => {
    ...
    return {
      ...
      variable: {
        ...
        availableUnitCount: candidates[index]?.availableUnitCount || 1
      }
    };
  }).filter(Boolean);
}
...
async function fetchExactSeries({ apiKey, country, productId, activityId, unit, frequency }) {
  ...
  if (points.length === 0) throw new Error("EIA returned metadata but no numeric observations for that series.");
...
function buildCandidateVariables(rows, intent, query) {
  ...
  for (const group of groups.values()) {
    ...
    const score = scoreVariable(candidate, intent, query) + Math.min(cleanRows.length, 20) * 0.1;
    const latest = cleanRows[cleanRows.length - 1];

    candidates.push({ ...candidate, label: `${group.productName} - ${group.activityName}`, latestPeriod: latest.period, latestValue: latest.value, observationsFound: cleanRows.length, score });
  }
...
function scoreVariable(group, intent, query) {
  const q = normalizeText(intent.correctedQuery || query);
  const product = normalizeText(group.productName);
  const activity = normalizeText(group.activityName);
  ...
  if (intent.product === "petroleum" && activity.includes("consumption") && isAggregatePetroleum(product)) score += 35;

  for (const word of q.split(" ")) if (word.length > 3 && hasPhrase(text, word)) score += 2;
  return score;
}

function isAggregatePetroleum(product) {
  return product.includes("petroleum and other liquids") || product.includes("total petroleum") || product.includes("refined petroleum products");
}
